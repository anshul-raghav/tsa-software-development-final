"""
Parses natural language user requests into structured TaskIntent.

Rules-based first (e.g. microwave heat_for_time, defrost); falls back to OpenAI for ambiguous input.
"""
...
from app.domain.models.task_models import TaskIntent
...
class IntentParsingService:
    ...
    async def parse(self, user_request: str, appliance_type: str) -> TaskIntent:
        ...
        try:
            ...
            if intent:
                logger.warning(f"OpenAI fallback failed, using rules-based result: {error}")
                return intent
            raise IntentParsingError(f"Could not parse intent: {error}")
