"""
Uses OpenAI Vision to extract a structured PanelMap from a panel image and OCR tokens.

Retries once on failure with a stricter prompt. Raises PanelMapExtractionError on parse/API errors.
"""
...
from app.domain.models.panel_map_models import PanelMap, OCRToken
...
class PanelMapExtractionService:
    ...
    USER_EXTRACTION_PROMPT = "Analyze this control panel image and return the structured PanelMap JSON."
    ...
    async def extract(
        ...
        try:
            ...
            logger.warning(f"First extraction attempt failed: {error}. Retrying...")
            try:
                response = await self._call_openai(image_b64, prompt, strict=True)
                return self._parse_response(response, scan_id)
            except Exception as retry_err:
                logger.error("PanelMap extraction failed after retry: %s", retry_err)
    ...
    async def _call_openai(self, image_b64: str, prompt: str, strict: bool = False) -> str:
        ...
        system_msg = prompt
        if strict:
            system_msg += self.STRICT_JSON_SUFFIX
            ...
            messages=[
                {"role": "system", "content": system_msg},
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": self.USER_EXTRACTION_PROMPT,
                        },
                    ],
                },
            ],
        ...
        if not content:
            logger.error("PanelMap extraction failed: empty response from OpenAI")
    ...
    def _parse_response(self, response: str, scan_id: str) -> PanelMap:
        try:
            ...
            logger.error("PanelMap extraction failed: invalid JSON from OpenAI: %s", error)
            raise PanelMapExtractionError(f"Invalid JSON from OpenAI: {error}")
        ...
        try:
            ...
            logger.error("PanelMap extraction failed: schema validation: %s", error)
            raise PanelMapExtractionError(f"PanelMap schema validation failed: {error}")
