...
# Ensure backend root is on path so "app" is importable when running pytest from any cwd
_backend_root = Path(__file__).resolve().parent.parent / "backend"
_backend_str = str(_backend_root.resolve())
if _backend_str not in sys.path:
    sys.path.insert(0, _backend_str)
# When running from backend dir, ensure cwd is on path (same as backend root)
_cwd = Path.cwd()
if _cwd.name == "backend" and str(_cwd.resolve()) not in sys.path:
    sys.path.insert(0, str(_cwd.resolve()))
