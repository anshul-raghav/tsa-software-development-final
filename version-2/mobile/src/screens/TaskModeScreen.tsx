...
import type { TaskPlan, TaskStep } from "../models/task";
...
export default function TaskModeScreen({
  ...
  const handleSubmitQuery = async () => {
    ...
    try {
      const result = await apiClient.planTask(scanId, query.trim());
      setTaskPlan(result.task_plan);
      setCurrentStepIndex(0);

      if (result.task_plan.clarification_needed) {
        await speechService.speak(result.task_plan.fallback_message);
      } else if (result.task_plan.steps.length > 0) {
        await speechService.speak(
          `I have ${result.task_plan.steps.length} steps for you. ` +
          `Step 1: ${result.task_plan.steps[0].instruction}`
        );
      }
