...
import { SCAN_SETTINGS } from "../constants/app-and-api-config";
...
export default function LiveGuidanceScreen({
  ...
  const startSession = async () => {
    if (!scanId?.trim() || !targetControlId?.trim()) {
      const msg = "Missing scan or target control. Please go back and try again.";
  ...
  const captureAndProcess = async (sessionId: string) => {
    if (!activeRef.current || processingRef.current || !cameraRef.current) {
      // Skip missed frames instead of hard failing; guidance should feel resilient.
    ...
    try {
      ...
      // Frame processing errors are non-fatal; a later frame often succeeds (lighting/motion/network).
    } finally {
      processingRef.current = false;
      scheduleNextFrame(sessionId);
    }
