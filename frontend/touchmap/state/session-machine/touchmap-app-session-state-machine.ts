...
export function transition(state: MachineState, event: SessionEvent): MachineState {
  ...
  switch (current) {
    ...
    case "requesting_permissions":
      // Permission success returns the user to the "safe" idle hub; screens decide what to render from there.
    ...
    case "panel_ready":
      // Record where we came from so Live Guidance can return to the correct mode without ad-hoc navigation state.
    ...
    case "live_guidance":
      if (event.type === "STOP_GUIDANCE") {
        // Guidance is a temporary overlay; return to the mode that initiated it.
